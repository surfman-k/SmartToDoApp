...
app.post('/newToDo', (data, res) => {
  ...
  placesPromises
    ...
    .then(function() {
      omdb({ t: input, apikey: process.env.omdbAPI })
        .list()
        .then(function(movie) {
...
app.post('/delete', (data, res) => {
  ...
  knex('todolist').where('id', data.body.id).del().then(function(error) {
    console.error(error);
  });
  res.redirect('/');
});
